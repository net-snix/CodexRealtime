...
const stripSearchAndHash = (value: string): { path: string; hash: string } => {
...
const parseFileUrlHref = (href: string): { path: string; hash: string } | null => {
  try {
...
const looksLikePosixFilesystemPath = (path: string) => {
  if (!path.startsWith("/")) {
    return false;
  }
...
const appendLineColumnFromHash = (path: string, hash: string) => {
  if (!hash || POSITION_SUFFIX_PATTERN.test(path)) {
    return path;
  }
...
const isLikelyPathCandidate = (path: string) => {
...
const hasExternalScheme = (path: string) => {
...
export const resolveMarkdownFileLinkTarget = (href: string | undefined, cwd?: string): string | null => {
  if (!href) {
    return null;
  }
